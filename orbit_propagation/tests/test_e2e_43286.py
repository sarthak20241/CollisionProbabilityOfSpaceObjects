...
for (norad_id, sample_points) in norad_samples.items():
	df_mc, mean_df_mc, execution_time_mc = mc_obj.propagate(sample_points, range(0,84000, 8400), norad_ids=norad_id)
	df_espt, mean_df_espt, execution_time_espt = espt_obj.propagate(sample_points, range(0,84000, 8400), norad_id=norad_id)
	df_aespt, mean_df_aespt, execution_time_aespt = espt_obj.propagate(sample_points, range(0,84000, 8400), norad_id=norad_id, propagation_type="aespt")
	...
	Visualize.covariance_ellipsoid([df_mc, df_espt, df_aespt], ["MC", "ESPT", "AESPT"], title="%s_covariance"%norad_id, file_name="%s_covariance"%norad_id, type='solid', alpha=0.3)
	Visualize.sample_point_visualize([df_mc, df_espt, df_aespt],  ["MC", "ESPT", "AESPT"], file_name="%s_samples"%norad_id, alpha=0.3)
